...
#1. Create random genom (weights)
def GenerateGenom(hidden_layer):
    W1_genom = np.random.randn(hidden_layer, X.shape[0])
    b1 = np.zeros(hidden_layer, 1)
    
    W2 = np.random.randn(Y.shape[0], hidden_layer)
    b2 = np.zeros(Y.shape[0], 1)

# GeerateGenom 50 times to create a population
...
#5. Implementing the evaluation/fitness calculation
...
for cnt in range(10000):
    ...
    y1 = forwardPropagation(sensors, W1, b1, W2, b2)
    print('y1:', y1)
